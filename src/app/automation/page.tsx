...
interface EditingPost {
  ...
  content: string;
  imageUrl?: string | null;
  scheduledAt: string; // Always a string for form input
  platform: string;
  campaignId?: string | null;
  campaign?: {
    id: string;
...
export default function AutomationPage() {
  ...
  const handleReschedulePost = () => {
    ...
    const scheduledAtDate = new Date(editingPost.scheduledAt);
  ...
  const openEditDialog = (post: ScheduledPost) => {
    const formattedDate = post.scheduledAt 
      ? new Date(post.scheduledAt).toISOString().split('T')[0] + 'T' + new Date(post.scheduledAt).toTimeString().slice(0, 5)
      : "";
